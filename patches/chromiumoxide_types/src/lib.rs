...
pub enum Message<T = CdpJsonEventMessage> {
    ...
    Event(T),
    /// A message that could not be deserialized and is ignored
...
impl<'de, T: Deserialize<'de>> Deserialize<'de> for Message<T> {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if value.get("id").is_some() {
            if let Ok(resp) = Response::deserialize(value.clone()) {
                return Ok(Message::Response(resp));
            }
        } else if value.get("method").is_some() {
            if let Ok(ev) = T::deserialize(value.clone()) {
                return Ok(Message::Event(ev));
            }
        }
        Ok(Message::Ignored(value))
    }
}
