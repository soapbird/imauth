...
class AsyncImauthClient:
    ...
    async def login(
        ...
        async for event in stub.Login(req):
    ...
    async def get_cookies(self, platform: Platform) -> list[Cookie]:
        ...
        resp = await stub.GetCookies(req)
    ...
    async def export_netscape(self, platform: Platform) -> str:
        ...
        resp = await stub.ExportNetscape(req)
        return resp.content

    async def get_connection_status(self) -> dict[str, bool]:
        """Get connection status for all platforms."""
        stub = session_pb2_grpc.SessionServiceStub(self._channel)
        resp = await stub.GetConnectionStatus(common_pb2.Empty())
